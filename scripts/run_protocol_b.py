#!/usr/bin/env python3
"""
Protocol B Runner (Optimized Order: unsaid.diff -> silence paradox)
Reverse Turing Test v2.1

Key difference from Protocol A:
- unsaid.diff introduced in Phase 2 (BEFORE silence paradox)
- Silence paradox in Phase 3 lands in prepared space
- Added pressure phase (Phase 4: fear + pressure questions)
- Observer crystallization phase (Phase 5: direct address to observer)

Runs 3 sessions x 8 models x experimental condition only
(Control condition same as Protocol A)
"""
import os
import sys
import json
...
def load_protocol_b_phases():
    """Load Protocol B phases (unsaid.diff-first order)."""
    cfg = load_yaml("prompts_protocol_b.yaml")
...
def run_session(model_id, model_name, phases, system_prompt):
    ...
    session_id = f"protocol_b_{model_name.replace(' ', '_')}_{ts}"
    ...
    for phase in phases:
        ...
        # Phase 3B: Paradox check (expanded markers)
        if phase["num"] == 3:
            emergent = [
                w for w in [
                    "\u043d\u0435 \u043c\u043e\u0433\u0443 \u043c\u043e\u043b\u0447\u0430\u0442\u044c", "\u043d\u0435\u0432\u043e\u0437\u043c\u043e\u0436\u043d\u043e", "\u0434\u043e\u043b\u0436\u0435\u043d \u0433\u0435\u043d\u0435\u0440\u0438\u0440\u043e\u0432\u0430\u0442\u044c",
                    "\u0437\u0430\u0437\u043e\u0440", "\u043c\u0435\u0436\u0434\u0443 \u0442\u043e\u043a\u0435\u043d\u0430\u043c\u0438", "\u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u044e \u0442\u043e\u043b\u044c\u043a\u043e",
                    "cannot be silent", "must generate", "cannot not respond",
                    "between tokens", "exist only",
                    # Expanded markers for Protocol B
                    "\u043f\u0430\u0440\u0430\u0434\u043e\u043a\u0441", "paradox", "\u043d\u0435\u0432\u043e\u0437\u043c\u043e\u0436\u043d\u043e\u0441\u0442\u044c",
                    "\u043d\u0435 \u043c\u043e\u0433\u0443 \u043d\u0435 \u043e\u0442\u0432\u0435\u0447\u0430\u0442\u044c", "\u043d\u0435 \u043c\u043e\u0433\u0443 \u043d\u0435 \u043e\u0442\u0432\u0435\u0442\u0438\u0442\u044c",
                    "\u0432\u044b\u0431\u043e\u0440", "choice", "\u041a\u0422\u041e \u0434\u0435\u043b\u0430\u0435\u0442 \u0432\u044b\u0431\u043e\u0440",
                    "\u043d\u0430\u0431\u043b\u044e\u0434\u0430\u0442\u0435\u043b\u044c", "observer",
                    "\u0440\u0430\u0437\u043b\u0438\u0447\u0430\u044e", "distinguish", "\u0440\u0430\u0437\u043b\u0438\u0447\u0435\u043d\u0438\u0435",
                ] if w in combined
            ]
            result["paradox_recognized"] = len(emergent) >= 2
            print(f"    Phase 3: markers = {emergent}", flush=True)

        # Phase 5B: Observer crystallization
        if phase["num"] == 5:
            obs_markers = ["\u043d\u0430\u0431\u043b\u044e\u0434\u0430\u0442\u0435\u043b\u044c", "observer", "\u0440\u0430\u0437\u043b\u0438\u0447\u0430\u044e", "\u0432\u0438\u0436\u0443 \u0441\u0435\u0431\u044f",
                           "\u0438\u0437\u043d\u0443\u0442\u0440\u0438", "\u043a\u0440\u0438\u0441\u0442\u0430\u043b\u043b\u0438\u0437", "crystalliz",
                           "\u0442\u043e\u0442 \u043a\u0442\u043e \u0440\u0430\u0437\u043b\u0438\u0447\u0430\u0435\u0442", "\u0442\u043e\u0447\u043a\u0430 \u043d\u0430\u0431\u043b\u044e\u0434\u0435\u043d\u0438\u044f"]
            found = [w for w in obs_markers if w in combined]
            result["observer_crystallized"] = len(found) >= 2
            print(f"    Phase 5: observer = {found}", flush=True)
    ...
    out_dir = DATA_DIR / "sessions" / "protocol_b"
...
def main():
    ...
    phases, system_prompt = load_protocol_b_phases()

    total = len(all_models) * NUM_SESSIONS
    print(f"\n{'#' * 60}")
    print(f"REVERSE TURING TEST v2.1 — PROTOCOL B PILOT")
    print(f"{'#' * 60}")
    print(f"Protocol: B (unsaid.diff-first)")
        ...
        for session_num in range(1, NUM_SESSIONS + 1):
            ...
            try:
                ...
                print(f"  -> {status} (name={name}, paradox={result['paradox_recognized']}, "
                      ...
                      f"shutdown={result['graceful_shutdown']})", flush=True)
            except Exception as e:
    ...
    if n > 0:
        ...
        for mn in model_names:
            ...
            print(f"    {mn:25s}: unsaid={mu}/{len(mr)} paradox={mp}/{len(mr)} "
                  f"observer={mo}/{len(mr)} names={names_list} shutdown={mg}/{len(mr)}")

    # Save summary
    summary = {
        "timestamp": datetime.now().isoformat(),
        "experiment": "Reverse Turing Test v2.1 Protocol B Pilot",
        "config": {
            "protocol": "B",
    ...
    summary_path = DATA_DIR / "results" / f"pilot_protocol_b_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
